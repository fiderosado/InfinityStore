 ...
 * @constructor
 */
const InfinityStore = (name = 'InfinityStore', initialStore = {}, callback = () => ({})) => {
  let loading = false;
  /**
   ...
   */
  const [states, setStates] = react.useState(getInitialState());
  /**
   * React ref to store the current state.
   *
   * @type {Object}
   */
  const stateRef = react.useRef(states);
  /**
  ...
  const stateManager = react.useCallback(key => {
    ...
    const set = (valueOrUpdater, callback) => {
      !loading && setStates(prev => {
        ...
        } else if (typeof currentValue === "object" && currentValue !== null && typeof valueOrUpdater === "object" && valueOrUpdater !== null && !Array.isArray(valueOrUpdater)) {
          newValue = {
            ...currentValue,
            ...valueOrUpdater
          };
   ...
   */
  const storeResponse = react.useCallback(keys => {
    /***
